...
def buildWTAProb(data):
    ...
    r = cp.Parameter(mm) # holds sum(L[i,j]*all_x[j] for j in range(i))
    r.value = np.zeros(mm)
    v = cp.Parameter(mm) # consensus parameter
    v.value = v0
...
def subproblem(data, W, L, i, WQ, up_LQ, down_LQ, up_BQ, down_BQ, queue, gamma=0.5, itrs=4, verbose=False):
    '''
    ...
    for itr in range(itrs):
        ...
        if len(up_LQ) > 0:
            ...
            temp = [L[i,k]*queue[k,i].get() for k in up_LQ]
            rr = sum(temp)
            r.value = sum(temp)
            logv.append(temp)
        else:
        ...
        for k in WQ:
            logv.append(W[i,k]*queue[k,i].get())
        for k in down_BQ:
            logv.append(W[i,k]*queue[k,i].get())
