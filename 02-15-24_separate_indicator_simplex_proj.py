...
def graphResults(logs, labels, ref):
    ...
    plt.ylabel('Value')
    plt.title('Projected WTA convergence: 4 nodes, 150 weapons, 270 targets, 200 iterations')
    ...
    # Save the figure
    timestamp = time()
    plt.savefig('projection_wta'+str(timestamp)+'.png')
...
if __name__ == "__main__":
    ...
    node_tgts = oars.distributeFunctionsLinear(n-1, tgts, num_nodes_per_function)
    num_nodes_per_tgt = num_nodes_per_function*np.ones(tgts) # Assumes even splitting
    v0 = 1/tgts*np.ones((tgts, wpns))
    # Generate the data
    data = smallSplit(n-1, tgts, wpns, node_tgts, num_nodes_per_tgt, L)
    ...
    #     print("node", i, "x")
    #     print(results[i]['w'])
    alg_p = fullValue(data[-1], proj_alg_x)
    print("alg val", alg_p)
    ...
    graphResults(logs, ["Node 1", "Node 2", "Node 3", "Projection"], true_p)
