...
def graphResults(logs, labels, ref, title):
    '''Graph the results'''
...
if __name__ == "__main__":
    ...
    #resolvents.append(simplexProj)
    t = time()
    alg_x, results = oars.solveMT(n, fulldata, resolvents, itrs=itrs, vartol= 1e-6, parallel=False, verbose=True)  
    ...
    graphResults(logs, None, true_p, 'LambertW Prox with alternating projection using MT')
